// ------------------------------------------------------------------------------------------------------
// <copyright file="BasescanService.cs" company="Nomis">
...
using Nomis.Basescan.Calculators;
...
{
    /// <inheritdoc cref="IBaseScoringService"/>
    internal sealed class BasescanService :
        BlockchainDescriptor,
        IBaseScoringService,
        IHasDefiLlamaIntegration,
        ITransientService
    {
        private readonly IBasescanClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="BasescanService"/>.
        /// </summary>
        /// <param name="settings"><see cref="BasescanSettings"/>.</param>
        /// <param name="client"><see cref="IBasescanClient"/>.</param>
        ...
        public BasescanService(
            IOptions<BasescanSettings> settings,
            IBasescanClient client,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "base";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            var transactions = (await _client.GetTransactionsAsync<BasescanAccountNormalTransactions, BasescanAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            var internalTransactions = (await _client.GetTransactionsAsync<BasescanAccountInternalTransactions, BasescanAccountInternalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            var erc20Tokens = (await _client.GetTransactionsAsync<BasescanAccountERC20TokenEvents, BasescanAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            var tokenTransfers = new List<IBasescanAccountNftTokenEvent>();
            var erc721Tokens = (await _client.GetTransactionsAsync<BasescanAccountERC721TokenEvents, BasescanAccountERC721TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            var erc1155Tokens = new List<IBasescanAccountNftTokenEvent>(); // (await _client.GetTransactionsAsync<BasescanAccountERC1155TokenEvents, BasescanAccountERC1155TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            var walletStats = new BaseStatCalculator(
                    request.Address,
